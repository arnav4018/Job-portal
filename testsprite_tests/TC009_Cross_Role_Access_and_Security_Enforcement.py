...
async def run_test():
    ...
    try:
        ...
        # Input candidate email and password, then click Sign In.
        frame = context.pages[-1]
        ...
        # Retry clicking the Sign In button to complete candidate login.
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/main/div/div/div[2]/form[2]/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Click 'Continue with Google' to attempt candidate login via Google OAuth.
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/main/div/div/div[2]/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Navigate to admin panel or attempt to access admin dashboard URL to test access restriction for candidate.
        await page.goto('http://localhost:3000/admin', timeout=10000)
        

        # Log out candidate user and login as expert user to test recruiter actions.
        await page.goto('http://localhost:3000/logout', timeout=10000)
        

        # Click on 'Sign In' to start login as expert user.
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/header/div/div[2]/div/a').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
