...
async def run_test():
    ...
    try:
        ...
        # Try clicking 'Continue with Google' to attempt alternative admin login method.
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/main/div/div/div[2]/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Try sending a magic link to admin email for alternative login.
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/main/div/div/div[2]/form[2]/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Input admin email into the email field on Vercel login page and click 'Continue with Email' button.
        frame = context.pages[-1]
